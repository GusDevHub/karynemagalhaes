...
<div class="conteudos">
    ...
    <?php while ( have_posts() ) : the_post(); //INÍCIO DO LOOP ?>
        ...
        <article class="col-xs-12 col-sm-8 col-md-8 interna-link nopaddl">
          ...
          <?php if( has_post_thumbnail( $post_id )){ ?>
              <?php the_post_thumbnail( 'post-imprensa', array( 'class'=>'img-responsive' ) ); ?>
            <?php } ?>
          <div class="imprensa-single-data">Publicado em <?php the_time('d/m\/Y\. ') ?></div>
  ...
  <div class="imprensa-single-outros-header col-xs-12 col-sm-4 col-md-4 col-lg-4">
    <h4>Posts Recentes</h4> <a href="saiu-na-imprensa" class="imprensa-single-link" title="ver todos">[+]</a>
  </div> 
  <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 nopadding wrap outros-artigos">
    <?php $currentID = get_the_ID(); $imprensa = new WP_Query( array( 'post_type'=>'saiu-na-imprensa','posts_per_page'   => 8, 'post__not_in' => array($currentID),'orderby' => 'date','order' => 'DESC' ) ); ?>
    <?php if( $imprensa->have_posts() ) : while( $imprensa->have_posts() ) : $imprensa->the_post(); ?>
        ...
        <div class="interna-link">
          <a href="<?php the_permalink(); ?>" title="<?php echo esc_html( the_title() ); ?>">       
            ...
            <div class="imprensa-single-data"><?php the_time('d/m\/Y\ ') ?></div>
